  ...
  return (
    <nav className="border-none w-dvs -mx-8 -mt-8 mb-8 p-4 flex items-center justify-between shadow-inner">
      ...
      <div className='flex space-x-8'>
        ...
        <NavbarLink url='/profile/test' label='My Profile' />
        {!session && <NavbarLink url='/login' label='Log In' />}
        {!!session &&
          <LogoutButton classNames={'font-medium text-stone-600 border-transparent bg-stone-300 rounded-full p-1 px-3 ml-2 flex flex-row space-x-2 hover:shadow-xl hover:bg-stone-400 hover:text-white text-sm'} text='Logout'/>
        }
      </div>
    </nav>
  )
